...
//listing04
template <class n_t>
inline rng_t operator^(const rng_t &r, const n_t &n) { 
  return rng_t(
    (r - n).first(), 
    (r + n).last()
  ); 
...
struct solver_2D
{
    ...
    bcy(j, i, adv_t::n_halos)
  {
    ...
    // redundant size (to make lower bounds of arrays equal)
...
struct cyclic
{
  ...
  cyclic(const rng_t &i, const rng_t &j, const int hlo) :
    //       (   i.first    ...     i.last         ), (  j ) 
    left_halo(rng_t(i.first()-hlo,  i.first()-1    ), j^hlo),
    rght_edge(rng_t(i.last()-hlo+1, i.last()       ), j^hlo),
    rght_halo(rng_t(i.last()+1,     i.last()+hlo   ), j^hlo),
    left_edge(rng_t(i.first(),      i.first()+hlo-1), j^hlo)
  {} 
...
inline auto donorcell_1D( 
  ...
  F(psi(pi<d>(i,  j)), psi(pi<d>(i+1,j)), C(pi<d>(i+h,j))) -
  F(psi(pi<d>(i-1,j)), psi(pi<d>(i,  j)), C(pi<d>(i-h,j)))
)
//listing10
...
template <int d>
inline auto A(
  const arr_t &psi, const rng_t &i, const rng_t &j
) return_macro(
  frac(
...
inline auto antidiff_2D(
  ...
  abs(C[d](pi<d>(i+h, j))) * (1 - abs(C[d](pi<d>(i+h, j)))) 
  * A<d>(psi, i, j) - C[d](pi<d>(i+h, j)) * .25 
  * (
    C[d-1](pi<d>(i+1, j+h)) + 
    C[d-1](pi<d>(i,   j+h)) +
    C[d-1](pi<d>(i+1, j-h)) + 
...
struct mpdata 
{
  ...
  inline void op_2D( 
    const arrvec_t &psi, const int n, const arrvec_t &C, 
    const rng_t &i, const rng_t &j, const int step
  ) {
    // chosing input/output for antidiff. velocity
    const arrvec_t 
      &C_unco = (step == 1) ? C : (step % 2) ? tmp0 : tmp1,
    ...
    if (step > 0) {
      const rng_t // ext. ranges as we only use C_+1/2 
        ...
        jm(j.first() - 1, j.last());
      C_corr[0](im + h, j ^ hlo) =
        antidiff_2D<0>(psi[n], im, j ^ hlo, C_unco);
      C_corr[1](i ^ hlo, jm + h) =
        antidiff_2D<1>(psi[n], jm, i ^ hlo, C_unco);
    }

...
\begin{document}
  ...
  The codes begins with the following definitions:
  ...
  All codes are strucutred in a way allowing compilation of the code 
...
%TODO: explain why h=(-1,0) is needed with Blitz and Python (in order not to lose memory)
  ...
  Python forbids overloading of operators of built-in types such as \prog{slices}, 
    and does not define addition/subtraction operators for \prog{slice}/\prog{int} pairs.
  ...
  In Fortran, similarly as in C++ and Python, the logic related to employment of the Arakawa-C grid
    allowing employment of fractional indices in array indexing is obtained through
...
%TODO: {discuss how halo region relates to parallel programming}
  ...
  The $\pi$ permutation is implemented in C++ as a function \prog{pi()}
    returning an instance of \prog{idx\_t} constructed with a permutation
    of the \prog{i} and \prog{j} arguments of \prog{pi()}, with permutation order
    dependant on the template parameter \prog{d}:
  ...
  In the Fortran implementation an analogous behaviour is obtained by introducing
    a function \prog{pi()} returning a pointer the array elements specified
...
%  TODO: templates -> branchless code by design, while F relies on opti
  ...
  The \prog{solver} structure is an abstract definition (containing a pure virtual method) 
     requiring its ancestors to implement at least

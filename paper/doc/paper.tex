...
\begin{document}
  ...
  Presented C++ implementation of MPDATA is built upon the Blitz++ 
    library\footnote{Blitz++ is a C++ class library for scientific computing which uses the expression templates technique to achieve high performance, see http://sf.net/projects/blitz/}.
  ...
  Arguably, the power of Blitz++ comes from the ability to express array expressions as objects.
  ...
  The \prog{arr\_t} type is defined solely for the purpose 
    of overcoming the Fortran's limitation of not allowing to declare arrays of arrays.
  The \prog{arrptr\_t} type is defined as creating an allocatable of its instances instead
    of multi-element pointer of \prog{arr\_t} ensures automatic memory deallocation.
...
%    the grid cell (finite volume) edges without interpolation.
  ...
  \prog{span()} is used to shorten the declarations of arrays to be returned 
  ...
  Consequently, while in C++ (and Fortran) the computational domain is chosen to start at (i=0, j=0)
    ...
    at (0,0)\footnote{The reason to allow the domain to begin at arbitrary index
  ...
  The above C++ and Python prototype solvers allow to operate with any boundary condition
    objects provided the requirements imposed by calls to these objects' methods are fulfilled 
  ...
  The above definition is not used in the following MPDATA implementation -- 
  ...
  The sequence of temporary arrays \prog{tmp} allocated in the constructor
  ...
  The first (step = 0) iteration of MPDATA is an unmodified donor-cell step (see listing C.15).
  ...
  The signals advected using MPDATA show smaller numerical diffusion.
  ...
  {\small
    ...
    \noindent
    Tobias Burnus, Ond\v{r}ej \v{C}ert\'ik, Julian Cummings, Patrik Jonsson,
      Arjen Markus, Zbigniew Piotrowski, Davide del Vento, Janus Weil and Andrzej Wyszogrodzki 
      provided valuable feedback to the initial version of the
      manuscript and/or responces to questions posted to Blitz++ and gfortran mailing lists.
